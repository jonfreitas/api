...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Inconsolata:wght@300;400;700&family=Lato:wght@300;400;700&display=swap" rel="stylesheet">
      ...
      {{else}}
        <li class="{{#if hidden}}hide {{/if}}" data-group="{{group}}" data-name="{{name}}" data-version="{{version}}">
          <a href="#api-{{group}}-{{name}}" title="{{url}}" data-group="show-api-{{group}}" data-name="show-api-{{group}}-{{name}}" class="nav-list-item show-api api-{{group}}-{{name}}-init">
            <span class="navtype navtype__{{toLowerCase type}}">{{toLowerCase type}}</span>
            {{title}}
            <div class="nav-list-url-item hide">{{url}}</div>
          </a>
        </li>
      {{/if}}
    {{/if}}
  ...
  <article id="api-{{article.group}}-{{article.name}}-{{article.version}}" {{#if hidden}}class="hide"{{/if}} data-group="{{article.group}}" data-name="{{article.name}}" data-version="{{article.version}}">
    <div class='article-content'>
      <div class='article-text'>
        <div class="pull-left">
          <h4 class='article-resource'>RESOURCE: {{underscoreToSpace article.groupTitle}}</h4>
          {{#if article.title}}<h1>{{article.title}}</h1>{{/if}}
        </div>
        {{#if template.withCompare}}
        <div class="pull-right">
          <div class="btn-group">
        ...
        {{#if article.deprecated}}
          <p class="deprecated">
            <span>{{__ "DEPRECATED"}}</span> {{{markdown article.deprecated.content}}}
          </p>
        {{/if}}
        ...
        {{/if}}
        <pre class="prettyprint language-html" data-type="{{toLowerCase article.type}}"><span class="type type__{{toLowerCase article.type}}">{{toLowerCase article.type}}</span><code>{{article.url}}</code></pre>
      ...
      <div class='article-code'>
        ...
        {{subTemplate "article-param-block-example" params=article.header _hasType=_hasTypeInHeaderFields section="header"}}
        {{subTemplate "article-param-block-example" params=article.parameter _hasType=_hasTypeInParameterFields section="parameter"}}
        {{subTemplate "article-param-block-example" params=article.success _hasType=_hasTypeInSuccessFields section="success"}}
        {{subTemplate "article-param-block-example" params=article.error _col1="Name" _hasType=_hasTypeInErrorFields section="error"}}

        <div class='collapse try-box' id="try-{{id}}">
          {{subTemplate "article-sample-request" article=article id=id}}
        </div>
        <button class='try-buttom' onclick='toggle("try-{{id}}")'>Try</button>
      </div>
    </div>
  </article>
</script>

<script id="template-article-param-block-example" type="text/x-handlebars-template">
  {{#if params}}
      ...
      <form class="form-horizontal">
        <fieldset>
            <div class="form-group">
              <label class="col-md-3 control-label" for="{{../id}}-sample-request-url">{{__ "url"}}</label>
              <div class="col-md-9">
